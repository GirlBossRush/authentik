...
/**
 * Recursively check if the target element is the active element.
 *
 * @param targetElement The element to check if it is the active element.
 * @param containerElement The container element to check if the target element is active in.
 */
function isActive(targetElement: Element | null, containerElement: Element | null): boolean {
    ...
    return isActive(containerElement.shadowRoot.activeElement, containerElement);
}
...
export class InputPassword extends AKElement {
    ...
    observeInputFocus(): void {
        this.inputFocusIntervalID = setInterval(() => {
            ...
            if (isActive(input, document.activeElement)) {
