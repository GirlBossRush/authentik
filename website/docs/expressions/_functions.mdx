...
### `ak_call_policy(name: str, **kwargs) -> PolicyResult`&nbsp;<VersionBadge semver="2021.12" />
...
### `ak_user_has_authenticator(user: User, device_type: Optional[str] = None) -> bool`&nbsp;<VersionBadge semver="2022.9" />
...
### `ak_create_event(action: str, **kwargs) -> None`&nbsp;<VersionBadge semver="2022.9" />
...
### `ak_create_jwt(user: User, provider: OAuth2Provider | str, scopes: list[str], validity = "seconds=60") -> str | None`&nbsp;<VersionBadge semver="2025.2" />
...
## DNS resolution and reverse DNS lookups&nbsp;<VersionBadge semver="2023.3" />
