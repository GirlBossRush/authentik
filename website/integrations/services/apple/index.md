...
Under **Advanced protocol settings**, make sure to add the newly created scope mappings to the provider:

- `ssf.manage`
- `ssf.read`
- `profile`

Visually confirm that you've copied the _Client ID_ and _Client Secret_ before pressing **Finish**.
...
Before leaving the new SSF provider overview page, make sure to copy the _SSF Config URL_ value. We'll need this when configuring Apple Business Manager.
...
With the application created, we're ready to switch to Apple Business Manager.
